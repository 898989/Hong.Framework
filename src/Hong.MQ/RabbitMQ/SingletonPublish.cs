...
　　  Topic : 将路由键和某模式进行匹配。此时队列需要绑定要一个模式上。符号“#”匹配一个或多个词，符号“*”匹配不多不少一个词。
    ...
    */
    ...
    /// <summary>发布
    /// </summary>
    public class SingletonPublish : RabbitMQBase, ISingletonPublish
    {
        ...
        public SingletonPublish(RabbitConfiguration config, ILoggerFactory loggerFactory = null) :
            base(config)
        {
            Log = loggerFactory?.CreateLogger("RabbitMQ.Publish");

            CreatePublishPropertys();
        ...
        void SendMsg(string queueKey, string msg)
        {
            ...
            Channel.BasicPublish(string.Empty, queueKey, PublishProperties, Encoding.UTF8.GetBytes(msg));
        ...
        void SendMsg(string exchange, string routingKey, string msg)
        {
            ...
            Channel.BasicPublish(exchange, routingKey, PublishProperties, Encoding.UTF8.GetBytes(msg));
